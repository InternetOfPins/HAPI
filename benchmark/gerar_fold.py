...
FLAGS_FIND   = [CXX, "-std=c++17", "-ftemplate-depth=2000",
                f"-I{INCLUDE_DIR}", "-I/usr/include"]
...
def runtime_ns(n, tmpdir, extra_sizes=False):
    ...
    for line in out.splitlines():
        for key in ("manual", "mutate", "ref", "trans", "hana"):
            if line.strip().startswith(key):
                try:
                    res[key] = float(line.split()[1])
...
transform_keys = ["baseline", "mutate", "trans", "hana_fold"]
transform_defs = {
...
ct_series = [
    ...
    ("mutate",    "Mutate<F>  (-std=c++20)", "firebrick",  "-",  "s"),
    ("trans",     "Trans<F>   (-std=c++20)", "darkorange", "-.", "^"),
    ("hana_fold", "Hana for_each",           "royalblue",  ":",  "D"),
]
...
find_series = [
    ...
    ("hana_first", "Hana find_if — first", "royalblue", "-",  "s"),
    ("hana_mid",   "Hana find_if — middle","royalblue", "-.", "D"),
    ("hana_last",  "Hana find_if — last",  "royalblue", ":",  "^"),
]
for key, label, color, ls, marker in find_series:
    ...
    ys = [y for y in ys if y is not None]
    ax.plot(xs, ys, label=label, color=color, linestyle=ls, marker=marker)
ax.set_title("Compile time — Find  (-fsyntax-only -O0, -std=c++17)")
ax.set_xlabel("N  (chain length)")
ax.set_ylabel("ms")
ax.legend(fontsize=9)
ax.grid(True, alpha=0.4)
...
rt_series = [
    ...
    ("mutate", "Mutate<F>",    "firebrick",  "-",  "s"),
    ("trans",  "Trans<F>",     "darkorange", "-.", "^"),
    ("hana",   "Hana for_each","royalblue",  ":",  "D"),
]
