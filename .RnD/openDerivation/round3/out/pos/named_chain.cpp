...
struct A  : hapi::APIOf<C,B> {using Base=hapi::APIOf<C,B>; using Base::Base; static_assert(hapi::Distinct<hapi::Chain<B,C>>, "duplicate layer in A");int get() const {return 1+Base::get();}};
...
template<typename T> struct W : hapi::APIOf<T,B> {using Base=hapi::APIOf<T,B>; using Base::Base; static_assert(hapi::Distinct<hapi::Chain<B,T>>, "duplicate layer in W");int get() const {return 100+Base::get();}};   // dependent base
struct K  {int v; K(int x):v(x) {} int get() const {return v;}};
struct AK : hapi::APIOf<K,B> {using Base=hapi::APIOf<K,B>; using Base::Base; static_assert(hapi::Distinct<hapi::Chain<B,K>>, "duplicate layer in AK");int get() const {return 1+Base::get();}};                          // K(int) reaches AK
