...
struct A  : B::Part<C> {using Base=B::Part<C>; using Base::Base;int get() const {return 1+Base::get();}};
...
template<typename T> struct W : B::template Part<T> {using Base=typename B::template Part<T>; using Base::Base;int get() const {return 100+Base::get();}};   // dependent base
struct K  {int v; K(int x):v(x) {} int get() const {return v;}};
struct AK : B::Part<K> {using Base=B::Part<K>; using Base::Base;int get() const {return 1+Base::get();}};                          // K(int) reaches AK
