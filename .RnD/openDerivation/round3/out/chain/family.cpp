...
struct AC : hapi::Chain<A>::Part<C> {using Base=hapi::Chain<A>::Part<C>; using Base::Base; static_assert(hapi::Distinct<hapi::Chain<A,C>>, "duplicate layer in AC"); static_assert(hapi::BuildRules<hapi::Chain<>,hapi::Chain<C,A>>::rules(), "HAPI: validation failed in AC");};
int main() {
