...
#include "identity.h"
struct G  : hapi::Chain<A,B>::Part<C> {using Base=hapi::Chain<A,B>::Part<C>; using Base::Base;};                                  // parenthesized right grouping: flattened
struct Y  : hapi::Chain<Any>::Part<X> {using Base=hapi::Chain<Any>::Part<X>; using Base::Base;};                                    // X (a named composition) as the last operand
struct Y2 : hapi::Chain<Any,A,B>::Part<C> {using Base=hapi::Chain<Any,A,B>::Part<C>; using Base::Base;};
template<typename... PP> struct F : hapi::Chain<PP...>::template Part<C> {using Base=typename hapi::Chain<PP...>::template Part<C>; using Base::Base;};
