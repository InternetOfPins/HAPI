...
| `godbolt/` | Banknote wave4 for Compiler Explorer, in the `:` form (translated) and the original `APIOf` form, including `single/hapi.h` by pinned raw URL; `verify.sh` checks both give static_net's `bnc_predict` | `godbolt/verify.sh` |
