"""
PlatformIO custom target for HLS synthesis via Intel HLS Compiler (i++).

Independent, third HLS backend cross-check for hls_can_disabler -- see
...
directly comparable to the Xilinx-targeted columns (same framing the
README already uses for the CAN-disabler-paper's Altera-vs-Xilinx
comparison).
...
env.AddCustomTarget(
    name="synthesize-can-disabler-intel",
    dependencies=None,
    actions=[_intel_hls_cmd(
        "canDisablerTop",
        os.path.join(HERE, "hls", "can_disabler_top.cpp"),
        os.path.join(HERE, ".hls_out_can_disabler_intel"),
    )],
    title="HLS: synthesize CAN transmit-disabler gate (Intel HLS Compiler)",
    description="Same design as synthesize-can-disabler, through Intel's "
                 "own HLS Compiler instead of Bambu -- independent tool "
                 "cross-check, DIFFERENT DEVICE FAMILY (Altera/Intel, not "
                 "Xilinx). See README.md.",
    always_build=True,
)
