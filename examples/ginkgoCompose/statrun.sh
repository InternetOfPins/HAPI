#!/usr/bin/env bash
# Paired, interleaved timing for the Round 2 delta. On a box without a
# fixed CPU governor, absolute ns/apply is noisy; the paired difference
# base_i - hapi_i is not (each pair runs back-to-back). Reports median +
# a trimmed mean over N trials. Pinned-core frequency logged per trial.
#
...
g++ -std=c++17 -O2 $INC -DHAPI src/stencil.cpp -o /tmp/gc_hapi $LIB

: > /tmp/gc_statrun.dat
for t in $(seq 1 "$TRIALS"); do
  f0=$([ -r "$FREQ" ] && cat "$FREQ" || echo 0)
  b=$(taskset -c $CORE /tmp/gc_base | awk '/ns\/apply/{print $2}')
  h=$(taskset -c $CORE /tmp/gc_hapi | awk '/ns\/apply/{print $2}')
...
print(f"n={n}  base median {st.median(b):.1f}  hapi median {st.median(h):.1f} ns")
print(f"paired base-hapi: median {st.median(d):.1f}  10%-trimmed mean {trim:.1f} ns")
