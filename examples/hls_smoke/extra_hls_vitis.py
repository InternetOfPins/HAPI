...
Independent, second HLS backend cross-check for hls_smoke -- same source,
same top function, same Xilinx part as the Bambu run, but a different
vendor's own tool instead of Bambu. See README.md's Cross-tool/
cross-config validation section.

    pio run -e hls-vitis -t synthesize-wrapsum-vitis
...
DEVICE = "xc7a100t-1csg324-VVD"  # same Artix-7 part as the Bambu run -- directly comparable
CLOCK_PERIOD = "10"
...
env.AddCustomTarget(
    name="synthesize-wrapsum-vitis",
    dependencies=None,
    actions=[_vitis_cmd(
        "wrapSum",
        os.path.join(HERE, "src", "main.cpp"),
        os.path.join(HERE, ".hls_out_wrapsum_vitis"),
    )],
    title="HLS: synthesize 5-layer Chain<> collapse (Vitis HLS)",
    description="Same design as synthesize-wrapsum, through AMD/Xilinx's "
                 "own Vitis HLS instead of Bambu -- independent tool "
                 "cross-check. See README.md.",
    always_build=True,
)
